...
dataset = tf.data.Dataset.from_generator(
    lambda: image_generator(image_paths, angles=[0, 90, 180, 270], target_size=(96, 120)),
    output_signature=tf.TensorSpec(shape=(96, 120, 3), dtype=tf.float32)
)
...
def make_generator_model():
    ...
    model.add(layers.Dense(12 * 15 * 256, use_bias=False, input_shape=(120,)))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    model.add(layers.Reshape((12, 15, 256)))
    ...
    model.add(layers.Conv2DTranspose(3, kernel_size=4, strides=1, padding='same', use_bias=False, activation='tanh'))
...
def make_discriminator_model():
    model = models.Sequential([
        layers.SpectralNormalization(layers.Conv2D(64, 5, strides=2, padding='same', input_shape=[96, 120, 3])),
        layers.LeakyReLU(),
        layers.Dropout(0.3),

        layers.SpectralNormalization(layers.Conv2D(128, 5, strides=2, padding='same')),
        layers.LeakyReLU(),
        layers.Dropout(0.3),

        layers.SpectralNormalization(layers.Conv2D(256, 5, strides=2, padding='same')),
        layers.LeakyReLU(),
        layers.Dropout(0.3),
...
def generate_and_save_images_separately(model, epoch, test_input, output_dir='generated_images'):
